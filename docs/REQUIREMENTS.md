...
ADR-0030 增加 `TAB-CLOSE-BATCH-001`：标签右键可关闭窗口全部标签、所选标签所在组的左侧或右侧标签，左右不包含所选标签，空范围禁用。需要处理的 dirty 文档包含历史引用，去重且排除仍被保留标签引用的文档；支持逐个保存/不保存或全部保存。所有必要处理完成前保留整批标签，取消保留未保存正文；保存失败、取消另存为、外部冲突及保存期间的新修改均阻止关闭，已成功保存的文件保持已保存。原生窗口退出同样可保存后退出。

`EDIT-CASE-001`：可视正文、源码和代码编辑区关闭系统自动首字母大写及自动纠正；新段落和句号后的英文保持输入的大小写，手动大写、中文组合输入、Undo/Redo 和既有正文不受影响。

ADR-0029 增加移动阅读验收：中央轻点及顶部按钮可随时呼出下方菜单，滑动/选字/组件操作不误触发；系统返回依次关闭浮层并返回文档/目录/工作区/连接页，首页连续 2 秒内两次返回退出。Markdown 相对路径与锚点在线和离线均可解析，失败保留当前页，最近 8 篇链接返回恢复各自位置。连接地址编辑保留身份，删除保留离线内容且不被发现结果立即加回。共享图片包含 SVG，单图 16 MiB，接近视口再加载；Mermaid 在本地按需渲染，错误/超时保留源码；查看器支持双指缩放和平移，关闭恢复原位置。表格列对齐、只读任务、引用式链接/图片、脚注和代码复制可用，公式维持现有语义。离线快照仍不保存图片，原始 HTML 不执行。原生系统返回的实际设备验收与自动测试分别记录。

ADR-0028 增加 `VIEW-READING-001`：默认编辑，顶部及专注状态均可一键切换阅读/编辑；开关作用于窗口内已有和新开的可视/源码/代码页面，不持久化。阅读时禁止正文输入、剪贴板修改、格式、任务/表格/图表编辑与替换，右键仅保留阅读动作；复制、查找、导航、查看器和导出可用，普通界面输入框可输入。切换保留当前表面、位置、Undo 与已有组合草稿及 dirty；不触发干净文档写盘，原有保存/关闭保护保持。

ADR-0027 接受以下稳定性修订；历史需求的验收语义以最新决策为准。

| ID                        | 状态 | 验收                                                                                                                                |
| ------------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `EDIT-BLOCK-ENTRY-001`    | Done | 移除可视正文左侧加号、块选择/拖动与跟随鼠标定位，右键仍可插入段落和结构操作；代码普通行号保留。                                     |
| `EDIT-UNDO-002`           | Done | 同 Tab/文档已访问表面保留 Undo/Redo，未访问表面惰性创建；隐藏时暂停更新和交互，切回同步最新正文且不回传旧内容。                     |
| `EDIT-MATH-CONTAINER-001` | Done | 引用、列表内 TeX 显示公式保持容器与公式体；编辑其他段落及分享导出不破坏语义。                                                       |
| `DOC-IDENTITY-001`        | Done | 大小写别名/符号链接打开共用规范路径身份；原子保存更新真实目标并保留链接，Save As 冲突保护继续有效。                                 |
| `NAV-RETENTION-001`       | Done | 每 Tab back/forward 各保留最近 200 项及必要 dirty/窗口轨迹所有权，无引用 clean 正文回收；超 1024 引用分批检查，超监听容量仍有兜底。 |
| `RENDER-PROJECTION-001`   | Done | 搜索直接按原生 UTF-16 片段范围高亮，不执行浏览器正则；正文未变的大纲不随光标/滚动重复解析和渲染。                                   |
| `MOBILE-RELIABILITY-001`  | Done | 不完整目录不能替换旧离线快照；重启后的最近阅读按稳定工作区与相对路径解析并继承位置；Release 按生命周期持有多播锁。                  |
...
| `MOBILE-OFFLINE-001`   | Active   | 用户逐工作区明确保存目录与完整 Markdown 正文到 IndexedDB；显示占用空间/最近同步时间，单快照限 128 MiB、5,000 目录和 5,000 文档，完整候选以单事务替换，失败保留旧快照。断网浏览、快照搜索和最近可用；进入/切换离线电脑或当前连接中断时仅短显提示，不随页面跳转重复；重连后标记工作区自动刷新，也可手动更新/清除。图片、附件和已渲染 Mermaid 资产本轮不缓存。                                                                                                                                                                                                                              |
...
| `EDIT-READ-STABLE-001`      | Done | 组件离屏回收不推动阅读中的正文                   | 代码、块公式、Mermaid 占位及同内容重载保留实测高度；停手后不因回收产生位移，不写正文或 Undo |
...
- 可视模式输入单星号不触发自动斜体：普通段落、列表及表格中的乘号保留，连续输入、输入法组合结束、Undo/Redo、源码往返和保存重开均不吞星号。已有 Markdown 斜体及显式格式菜单/快捷键继续可用，双星号粗体与行首星号加空格创建列表保持原行为。
...
- 图片与 Mermaid 查看器的缩放比例须对应实际显示尺寸，SVG 放大保持矢量；手动缩放后重复加载、父组件更新和窗口尺寸变化不得自动缩回。滚轮与 Tab/Shift+Tab 不得带动被遮住的正文，关闭保持原阅读位置，加载失败时仍可通过键盘关闭。Mermaid 默认配色支持时序图，源码指定的主题及颜色保留且不影响后续图表。
...
- 离线工作区显示占用空间和最近同步时间。进入离线阅读、切换到另一台离线电脑或当前连接中断时短暂提示一次，随后只保留紧凑状态标识，不以常驻提示框占用目录或正文空间；普通重渲染、目录跳转和文档跳转不得重新触发，恢复在线立即清除。连接对应电脑后，仍标记为离线保存且当前可见的工作区自动刷新，也支持手动刷新/清除。离线浏览、快照正文搜索和最近阅读可用；图片、附件与已渲染 Mermaid 资产本轮不进入离线包，Markdown 内 Mermaid 源码仍随正文保存，但离线时不得伪装渲染资源已缓存。
...
| `MOBILE-OFFLINE-NOTICE-001` | 进入/切换离线电脑或当前连接中断只短显；导航不重复，在线即清除，不保留常驻提示框                     |
...
| ID                     | 目标                                                                                                  |
| ---------------------- | ----------------------------------------------------------------------------------------------------- |
| `PERF-TYPE-001`        | 普通输入和 IME 无明显掉帧或光标跳动；组合选区反复展开/折叠不得改变首块间距、逐字推动正文              |
...
| `AC-LAN-OFFLINE-001`       | 离线快照可浏览/搜索/最近；提示短显且导航不重复；重连原子刷新，失败保留旧版；上限与资源排除正确                                 |
