...
| `P0-CONTRACT-01` | CLAIMED | Contract/Domain (`/root/p0_contract_01`) | original `576a435`; third-review rework base `91a2782` | `DATA-REVISION-001`、`SAFE-IPC-001`、`EXT-ROUTER-001`、`EXT-COMMAND-001` / IPC `1.0-draft` Rust-derived schema | [`P0-CONTRACT-01.md`](tasks/P0-CONTRACT-01.md)；第三轮独立复审判定 NO-MERGE：修复嵌套 AppError 规范化、DocumentExternalChanged 严格 serde、Rust 派生 event validator 与机械 union fixture 覆盖；既有通过证据在新实现完成前撤销，`004`–`024` 仍为 frozen ports |
