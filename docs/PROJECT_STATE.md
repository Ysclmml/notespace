...
| `P0-CONTRACT-01` | CLAIMED | Contract/Domain (`/root/p0_contract_01`) / same owner | fourth-review base `556b279` / claim checkpoint pending | `DATA-REVISION-001`、`SAFE-IPC-001`、`EXT-ROUTER-001`、`EXT-COMMAND-001` / IPC `1.0-draft` AppError schema parity rework | [`P0-CONTRACT-01.md`](tasks/P0-CONTRACT-01.md)；baseline docs PASS，canonical runner Rust 14/14 + TS 24/24 PASS；修复 direct/nested AppError Rust-schema parity 与 Rust-derived recovery-action registry 后重交 REVIEW，`004`–`024` 保持 frozen ports |
