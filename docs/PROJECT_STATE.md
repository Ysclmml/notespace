...
| `P0-CONTRACT-01` | REVIEW | Contract/Domain (`/root/p0_contract_01`) / Integration + independent Contract reviewer | fourth-review base `556b279` / implementation `154d48c` | `DATA-REVISION-001`、`SAFE-IPC-001`、`EXT-ROUTER-001`、`EXT-COMMAND-001` / IPC `1.0-draft` Rust-derived AppError/runtime policy | [`P0-CONTRACT-01.md`](tasks/P0-CONTRACT-01.md)；standalone AppError schema、Rust 单源 recovery registries、direct/nested parity 与字段 mutation PASS；canonical Rust 15/15 + TS 29/29、full verify、dependency/privacy 与 clean clone PASS；下一步独立复审并由 Integration 接入 runner，`004`–`024` 保持 frozen ports |
