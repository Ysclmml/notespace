...
| `EDT-LIVE-001` / `EDIT-LIVE-001` | Cursor-local complete source range is revealed without changing `EditorState.doc` | PASS for heading/link/image/fenced-code/emphasis/table ranges, cursor, and non-empty selection; table outranks nested link/inline-code and reveals the complete table source; list/quote markers stay visible because this spike has no safe `ListItem`/`Blockquote` reveal range |
...
- Added eighteen focused tests covering cursor and non-empty selection, unknown/incomplete syntax, exact pipe-table source, structural ancestor priority, composition DOM-event ordering, undo/redo, cancellation, range normalization, proven-local scanning, cross-line syntax propagation, list/quote marker safety, and generator failure boundaries.
...
- Closed the second-review blocker by removing `ListMark`/`QuoteMark` from replacement rather than pretending the spike implements list/quote live preview. Cursor-current, cursor-non-current, composing, `refreshPending`, and post-RAF regressions all assert those markers are never replaced.
...
- List and blockquote live preview is outside this disposable spike. Until a future implementation defines complete `ListItem`/`Blockquote` source ranges and dedicated behavior tests, their markers remain ordinary visible source and are not replacement candidates.
...
- The final post-review measurement process retained 309,182,464 B RSS after repeatedly constructing/destroying all 30-sample scenarios without forced GC. This is below the 512 MiB spike ceiling but is not a memory-plateau proof; `P1-EDITOR-01`/`P5-LARGE-01` must measure real WebKit lifecycle and multiple views.
