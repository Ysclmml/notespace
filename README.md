...
- **直接编辑排版后的 Markdown**：标题、列表和表格保持可视形态，光标进入时不自动展开源码；保留输入的字母大小写，关闭自动大写和自动纠正。输入单星号乘法表达式时保留原文，斜体可通过菜单或快捷键设置。需要精确调整标记时，可主动切换源码模式。
...
- **临时标签与分屏对照**：单击文件临时预览，双击或编辑后保持打开；标签可在分屏之间拖动。右键可关闭全部或左侧、右侧标签，未保存内容可逐个处理或全部保存后关闭。文中代码引用可按行预览，或在右侧打开，与笔记并排阅读、编辑。
- **截图与图表融入写作**：粘贴截图先保存为本地图片，再插入引用；每个工作区可指定图片目录。图片与 Mermaid 图表可缩放、平移，矢量图放大保持清晰，关闭查看器后继续原来的阅读位置。
...
- **手机局域网与离线阅读**：浏览桌面共享的 Markdown、公式、图片和 Mermaid 图表，点击文档链接继续阅读；轻点正文呼出菜单，返回键逐层返回。可修改或删除电脑连接，完整 Markdown 离线副本让断网后仍可阅读文字、公式与图表。
...
Android 阅读端支持共享目录内的图片、Mermaid 图表和 Markdown 链接跳转，代码可直接复制。轻点正文中间呼出下方阅读菜单，系统返回逐层返回，首页连续两次返回才退出；电脑连接可修改地址或删除。图片与附件不进入离线副本，Mermaid 可在手机本地从已保存的正文渲染；SVG 图片需要桌面端和手机端均更新至 0.2.4 或更新版本。
