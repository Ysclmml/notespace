...
- **工作区全文搜索**：左侧「工作区搜索」或 `Cmd/Ctrl+Shift+F`，按内容查找多个目录中的文档与代码，点击结果在当前分屏定位。搜索磁盘文件，不包含未保存修改；过大文件和扫描上限会提示。
- **HTML 导出**：在顶部「文件」菜单或右上角更多菜单导出当前 Markdown，得到带排版样式的阅读页面。导出包括当前未保存编辑，不修改原文件。图片保持原位置引用，Mermaid 保留源码；单篇正文上限为 8 MiB，暂不打包资源或导出 source-only 文档。
