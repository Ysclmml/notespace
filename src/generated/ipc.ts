...
export const READ_ONLY_RECOVERY_ACTIONS = IPC_EVENT_PAYLOAD_SCHEMAS.readOnlyRecoveryActions;
...
function isKnownRecoveryAction(value: unknown): value is RecoveryAction {
  return (
    typeof value === "string" && (RECOVERY_ACTIONS as readonly string[]).includes(value)
  );
}
...
function decodeSchemaInternal(
  ...
  if (allOf !== undefined) {
    ...
    for (const branch of allOf) {
      ...
      decoded = decodeSchemaInternal(decoded, branch, root, normalizeAppErrorReferences);
      if (decoded === INVALID_SCHEMA_VALUE) return INVALID_SCHEMA_VALUE;
    }
  }
